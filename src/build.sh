...
cat ../picprog.ino | sed -n '/^\/\* BEGIN HEX SECTION \*\//q;p' | sed "s/^#define STC1000PI_VERSION.*/$v/" | sed "s/^#define STC1000PI_EEPROM_REV.*/$e/" >> picprog.tmp

# Insert new HEX data
echo "/* BEGIN HEX SECTION */" >> picprog.tmp; 
echo "#ifdef CELSIUS" >> picprog.tmp; 

echo "const char hex_autotune[] PROGMEM = {" >> picprog.tmp; 
for l in `cat build/stc1000pi_at_celsius.hex | sed 's/^://' | sed 's/\(..\)/0\x\1\,/g'`; do 
	echo "   $l" | sed 's/0x00,0x00,0x00,0x01,0xFF,/0x00,0x00,0x00,0x01,0xFF/' >> picprog.tmp; 
done; 
echo "};" >> picprog.tmp

echo "const char hex_pid[] PROGMEM = {" >> picprog.tmp; 
for l in `cat build/stc1000pi_celsius.hex | sed 's/^://' | sed 's/\(..\)/0\x\1\,/g'`; do 
	echo "   $l" | sed 's/0x00,0x00,0x00,0x01,0xFF,/0x00,0x00,0x00,0x01,0xFF/' >> picprog.tmp; 
done; 
echo "};" >> picprog.tmp

echo "const char hex_eeprom[] PROGMEM = {" >> picprog.tmp; 
for l in `cat build/eedata_celsius.hex | sed 's/^://' | sed 's/\(..\)/0\x\1\,/g'`; do 
	echo "   $l" | sed 's/0x00,0x00,0x00,0x01,0xFF,/0x00,0x00,0x00,0x01,0xFF/' >> picprog.tmp; 
done; 
echo "};" >> picprog.tmp

echo "#endif // CELSIUS" >> picprog.tmp; 

echo "#ifdef FAHRENHEIT" >> picprog.tmp; 

echo "const char hex_autotune[] PROGMEM = {" >> picprog.tmp; 
for l in `cat build/stc1000pi_at_fahrenheit.hex | sed 's/^://' | sed 's/\(..\)/0\x\1\,/g'`; do 
	echo "   $l" | sed 's/0x00,0x00,0x00,0x01,0xFF,/0x00,0x00,0x00,0x01,0xFF/' >> picprog.tmp; 
done; 
echo "};" >> picprog.tmp
...
echo "#endif // FAHRENHEIT" >> picprog.tmp; 
...
echo "Autotune size"
let s=0;
for a in `cat build/page0at_c.asm build/page1at_c.asm | grep instructions | sed 's/^.*=  //' | sed 's/ instructions.*//'`;
do
	echo $a;
	s=$(($s+$a));
done;
